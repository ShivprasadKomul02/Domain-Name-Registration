...
/*
Solution 2: preRegistering domainname
...
contract DNS_ADV is Ownable{
   ...
   //modifier to check calling from domainName owner or not
    ...
    //checks if user preRegistered the domainname or not
    modifier isRegistrationOpen(address _sender, string memory _vName) {
		...
		require(preRegisters[secret] > 0,"domain Name not preregistered");
  ...
  function bytes32ToString(bytes32 _bytes32) public pure returns (string memory) {
    ...
    function regiterdomainName(string memory _vName,uint _txCounter) public payable 
    ...
    //function preregsiters the domain name to avoid frontrunning

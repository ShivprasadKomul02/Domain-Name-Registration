...
import "http://github.com/OpenZeppelin/openzeppelin-solidity/contracts/ownership/Ownable.sol"; 
    ...
    1.when user registers domain name by entering transaction counter no 
    2.Then registerdomainName trnsaction gets included in transaction(mem) pool of miners.
    ...
    4.so owners transaction will mined first before user registration for domain name
    5.then txCounter is increased when owners transaction gets mined first
    5.when the users domainNameRgistration transaction trying to confirm it gets failed
...
contract DNS_ADV is Ownable{
  ...
  function bytes32ToString(bytes32 _bytes32) public pure returns (string memory) {
    ...
    function regiterdomainName(string memory _vName,uint _txCounter) public payable isValidName(_vName) isNameAvailable(_vName) isMoneyAvailable(_vName)
    {
    ...
    function renewdomainName(string memory _vName,uint _txCounter) public payable isNameOwner(_vName) isMoneyAvailable(_vName)
    {
        require(txCounter==_txCounter,"Frontrunning");
         string memory vNameHash=bytes32ToString(encrypt(_vName));
         uint finalPrice=calculatePrice(_vName);
